...
| | | | \
|向量检索  |SearchByVector  |\
| |dense_vector参数  |[https://www.volcengine.com/docs/84313/1419285](https://www.volcengine.com/docs/84313/1419285) |\
| | |设置order_by_vector.vectors参数  |
| | | | \
|稠密稀疏向量混合检索  |SearchByVector，  |\
| |设置sparse_vector参数  |[https://www.volcengine.com/docs/84313/1419286](https://www.volcengine.com/docs/84313/1419286) |\
| | |设置order_by_vector.sparse_vectors参数  |
| | | | \
|向量检索下支持张量重排  |SearchByVector  |\
| |设置 tensor_rerank 参数，详见：[Tensor Rerank 结构 ](/docs/84313/1791133#d67b9963) |不支持  |
| | | | \
|多模态检索  |SearchByMultiModal  |\
| |其中，若为支持instruction的模型，需要主动设置是否添加instruction，以增强感知度。请求结果中会返回拼接instruction后的query语句。  |[https://www.volcengine.com/docs/84313/1419288](https://www.volcengine.com/docs/84313/1419288) |\
| | |设置order_by_raw参数  |
| | | | \
|多模态检索下支持张量重排  |SearchByMultiModal  |\
| |设置 tensor_rerank 参数，详见：[Tensor Rerank 结构 ](/docs/84313/1791133#d67b9963) |不支持  |
| | | | \
|主键检索  |SearchByID  |\
| |  |[https://www.volcengine.com/docs/84313/1419285](https://www.volcengine.com/docs/84313/1419285) |\
| | |设置order_by_vector.primary_keys参数  |
| | | | \
|标量排序检索  |SearchByScalar  |[https://www.volcengine.com/docs/84313/1419287](https://www.volcengine.com/docs/84313/1419287) |\
| | |设置order_by_scalar参数  |
| | | | \
|随机检索  |SearchByRandom  |\
| |  |[https://www.volcengine.com/docs/84313/1578505](https://www.volcengine.com/docs/84313/1578505) |\
| | |不设置任何order_by_xxx参数  |
| | | | \
|关键词检索  |SearchByKeywords  |不支持  |
| | | | \
|主键过滤  |ids_in、ids_not_in参数  |\
| |  |primary_key_in、primary_key_not_in参数  |
| | | | \
...
|---|---|---|---|
| | | | | \
|must |string、int64、bool、list<string>、list<int64> |针对指定字段名生效，语义为必须在 [...] 之中，即 "must in" |\
| | |“conds”中的每个条件之间是 or 关系  |```JSON |\
| | | |{ |\
| | | |  "op": "must",     |\
| | | |  "field": "region",  |\
| | | |  "conds": ["cn", "sg"] |\
...
| | | | |
| | | | | \
|time_range |date_time |时间点筛选，配置使用`gte`（大于等于）, `gt`（大于）, `lte`（小于等于）, `lt`（小于），用以圈定一维范围。 | |\
| | | |```JSON |\
| | | |{ |\
...
| | | | |
| | | | | \
|geo_range |geo_point |地理距离筛选，使用 `center `为中心点，`radius `为距离半径。 |\
| | | | |\
| | | |```JSON |\
| | | |{ |\
| | | |    "collection_name": "test", |\
...
|---|---|---|---|
| | | | | \
|score_fusion |string |分数融合算子，用于融合指定的时空标量的分数。 |```Scala |\
| | | |{ |\
| | | |        "op": "score_fusion", |\
| | | |        "fusion_by": "add", |\
