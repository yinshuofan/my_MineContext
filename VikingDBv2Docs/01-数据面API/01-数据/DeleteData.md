...
**若数据集带vectorize字段（选择“从向量化开始”）：​**请根据需求上传text、image和video字段中的一个或多个，不要上传“vector”字段；
...
| | | | \
|单次写入的数据量限制 |如果数据集是带vectorize的，不超过1 条 / 次请求，时延更小；不带vectorize的，不超过100 条 / 次请求 。 |不超过100。 |
...
| | | | \
|sparse_vector |\
| |* 输入格式<token_id ,token_weight>的字典列表，来表征稀疏稀疏向量的非零位下标及其对应的值 |\
| |* 其中 token_id 是 string 类型, token_weight 是float 类型 |稀疏向量 |
| | | | \
|tensor |* 张量：输入格式为 M*N 的浮点二维数组（向量矩阵） |\
| | |\
| | |张量 |\
| | |例如：以shape为4*8的二维数组格式为例（仅示例） |\
| | |```Plain Text |\
| | |tensor = [ |\
