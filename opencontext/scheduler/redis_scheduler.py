...
"""
Redis-backed Task Scheduler Implementation

A stateless task scheduler that stores all state in Redis,
supporting multi-instance deployment.
"""
...
class RedisTaskScheduler(ITaskScheduler):
    ...
    def __init__(
        ...
        # Initialize task types from config
        self._init_task_types()
    
    def _init_task_types(self) -> None:
        """Initialize task types from configuration"""
        ...
        for task_name, task_config in tasks_config.items():
            if task_config.get("enabled", False):
                ...
                self.register_task_type(config)
    ...
    def get_pending_task(self, task_type: str) -> Optional[TaskInfo]:
        ...
        for user_key in tasks:
            ...
            # Update task status
            task_key = f"{self.TASK_PREFIX}{task_type}:{user_key}"
            self._redis.hset(task_key, "status", TaskStatus.RUNNING.value)
    ...
    def complete_task(
        ...
        self._redis.hset(task_key, "status", status.value)
        
        # Record execution time
        if success:
            self._redis.set(last_exec_key, str(int(time.time())))
            self._redis.expire(last_exec_key, 86400)  # 24 hours
    ...
    async def _process_task_type(self, task_type: str) -> None:
        ...
        task_info = self.get_pending_task(task_type)
        if not task_info:
            return
    ...
    async def _process_periodic_tasks(self) -> None:
        """Process periodic tasks (global tasks, not user-specific)"""
        tasks_config = self._config.get("tasks", {})
        ...
        for task_type, task_config in tasks_config.items():
            ...
            try:
                ...
                self._redis.hset(periodic_key, "status", "failed")
            finally:
                self._redis.release_lock(lock_key, lock_token)
