...
Low-level client wrapping `openai.AsyncOpenAI`. All methods are async. Instantiated with an `LLMType` (CHAT or EMBEDDING) that gates which methods are callable.

**Constructor:**
```python
def __init__(self, llm_type: LLMType, config: Dict[str, Any])
```
Fields: `self.model`, `self.api_key`, `self.base_url`, `self.timeout` (default 300), `self.max_retries` (default 3, SDK built-in retry for 429/5xx with exponential backoff), `self.provider` (default `"openai"`), `self.client` (`AsyncOpenAI` instance).

**Chat methods** (require `LLMType.CHAT`):

| Method | Signature | Returns |
|--------|-----------|---------|
| `generate_with_messages` | `async (messages: List[Dict[str, Any]], **kwargs)` | OpenAI `ChatCompletion` response object |
| `generate_with_messages_stream` | `async generator (messages: List[Dict[str, Any]], **kwargs)` | Async generator yielding chunks |
...
| Method | Signature | Returns |
|--------|-----------|---------|
| `generate_embedding` | `async (text: str, **kwargs) -> List[float]` | Single-text embedding vector |
| `generate_embedding_batch` | `async (texts: List[str], **kwargs) -> List[List[float]]` | Batch embedding; internally chunks by `max_batch_size` (default 64, configurable); falls back to individual calls on error |
| `vectorize` | `async (vectorize: Vectorize, **kwargs) -> None` | Sets `vectorize.vector` in-place |
...
- `LLMProvider`: `OPENAI`, `DOUBAO`, `DASHSCOPE`
...
| `generate_with_messages` | `async (messages, enable_executor=True, max_calls=5, **kwargs) -> str` | Async chat with auto tool-call loop; parallel tool execution via `asyncio.gather` |
| `generate_for_agent_async` | `async (messages, tools=None, **kwargs)` | Returns raw response without auto-executing tool calls |
| `generate_stream_for_agent` | `async generator (messages, tools=None, **kwargs)` | Async generator; yields stream chunks |
| `execute_tool_async` | `async (tool_call) -> Any` | Executes a single OpenAI-format tool call |

**Tool-call loop logic:** After each LLM response, if `message.tool_calls` exists and `enable_executor=True`, executes all tool calls in parallel via `asyncio.gather`, appends results as `role: "tool"` messages, and re-calls the LLM. Stops after `max_calls` iterations or when no tool calls are returned.

**Module-level convenience functions:** `is_initialized()`, `generate_with_messages()`, `generate_for_agent_async()`, `generate_stream_for_agent()`. All except `is_initialized` are async.
