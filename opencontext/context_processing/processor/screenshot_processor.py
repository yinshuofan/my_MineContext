...
class ScreenshotProcessor(BaseContextProcessor):
    ...
    def _get_processed_cache_key(
        self, context_type: str, user_id: str = "default", device_id: str = "default"
    ) -> str:
    ...
    def get_description(self) -> str:
        ...
        return (
            "Stateless screenshot processor with Redis state externalization and async processing."
        )
    ...
    def _is_duplicate_redis(
        self, phash: str, user_id: str = "default", device_id: str = "default"
    ) -> bool:
        """
    ...
    def _set_cached_contexts(
        self,
        context_type: str,
        contexts: Dict[str, ProcessedContext],
        user_id: str = "default",
        ...
        try:
            ...
            try:
                ...
                for ctx in processed_contexts:
                    increment_data_count(
                        "context", count=1, context_type=ctx.extracted_data.context_type.value
                    )

                increment_recording_stat("processed", len(processed_contexts))
            except Exception as e:
                logger.debug(f"Failed to record screenshot metrics: {e}")
    ...
    async def _process_single_screenshot(
        self,
    ...
    async def _merge_contexts(
        ...
        for context_type, new_items in items_by_type.items():
            ...
            tasks.append(
                self._merge_items_with_llm(
                    context_type, new_items, cached_items, user_id, device_id
                )
            )
        ...
        for result in response_data.get("items", []):
            ...
            if merge_type == "merged":
                ...
                if not merged_ids or merged_ids[0] not in all_items_map:
                    logger.error(
                        "new type but no merged_ids or merged_ids[0] not in all_items_map, skipping"
                    )
                    continue
