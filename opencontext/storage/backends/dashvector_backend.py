    ...
    def request(
        ...
        if use_async and AIOHTTP_AVAILABLE:
            ...
            try:
                ...
                return asyncio.ensure_future(self._async_request(method, path, data, params))
            except RuntimeError:
            ...
            try:
                ...
                if loop and loop.is_running():
                    ...
                    try:
                        ...
                        # If we can't get a loop and can't run a new one, just ignore
                        pass
            except Exception as e:
                logger.debug(f"Error closing DashVector async session: {e}")
...
class DashVectorBackend(IVectorStorageBackend):
    ...
    def initialize(self, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"DashVector HTTP backend initialized with {len(self._collections)} collections"
            )
            return True
    ...
    def _wait_for_collection_ready(self, collection_name: str, max_wait: int = 60) -> bool:
        """
        ...
        while time.time() - start_time < max_wait:
            try:
                result = self._client.request("GET", f"collections/{collection_name}/stats")
                if result.get("code") == 0:
                    return True
            except Exception as e:
                logger.debug(f"DashVector health check attempt failed: {e}")
            time.sleep(1)
        ...
        for context_type, type_contexts in contexts_by_type.items():
            ...
            try:
                result = self._client.request(
                    "POST", f"collections/{context_type}/docs/upsert", data={"docs": docs}
                )
                ...
                if result.get("code") == 0:
                    ...
                    logger.error(f"Failed to upsert to {context_type}: {result.get('message')}")
    ...
    def get_processed_context(
        ...
        try:
            ...
            result = self._client.request(
                "GET", f"collections/{context_type}/docs", params={"ids": id}
            )
    ...
    def get_all_processed_contexts(
        ...
        target_types = (
            context_types
            if context_types
            else [ct for ct in self._collections.keys() if ct != TODO_COLLECTION]
        )
        ...
        for ctx_type in target_types:
            ...
            try:
                ...
                query_result = self._client.request(
                    "POST", f"collections/{ctx_type}/query", data=data
                )
    ...
    def delete_contexts(self, ids: List[str], context_type: str) -> bool:
        ...
        try:
            result = self._client.request(
                "DELETE", f"collections/{context_type}/docs", data={"ids": ids}
            )
    ...
    def search(
        ...
        for ctx_type in target_types:
            try:
                ...
                result = self._client.request("POST", f"collections/{ctx_type}/query", data=data)
    ...
    def get_processed_context_count(self, context_type: str) -> int:
        ...
        try:
            result = self._client.request("GET", f"collections/{context_type}/stats")
            if result.get("code") == 0:
                output = result.get("output", {})
    ...
    def upsert_todo_embedding(
        ...
        try:
            ...
            result = self._client.request(
                "POST", f"collections/{TODO_COLLECTION}/docs/upsert", data={"docs": [doc]}
            )
    ...
    def search_similar_todos(
        ...
        try:
            ...
            result = self._client.request("POST", f"collections/{TODO_COLLECTION}/query", data=data)
    ...
    def delete_todo_embedding(self, todo_id: str) -> bool:
        ...
        try:
            result = self._client.request(
                "DELETE", f"collections/{TODO_COLLECTION}/docs", data={"ids": [todo_id]}
            )
            return result.get("code") == 0
